...
export default function AdminEventiPage() {
  ...
  async function load() {
    ...
    setMsg(null);
    try {
      const r = await fetch("/api/admin/eventi", { headers: { authorization: `Bearer ${token()}` } });
  ...
  async function upload() {
    ...
    fd.append("file", file);
    const r = await fetch("/api/admin/eventi", { method: "POST", headers: { authorization: `Bearer ${token()}` }, body: fd });
    const j = await r.json();
  ...
  async function del(name: string) {
    setMsg(null);
    const r = await fetch(`/api/admin/eventi?name=${encodeURIComponent(name)}`, { method: "DELETE", headers: { authorization: `Bearer ${token()}` } });
    const j = await r.json();
